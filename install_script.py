...
# Define the source DIRECTORY containing the main DLL and its dependencies
source_plugin_dir: Path = Path('/home/amp/')
# Define the path to the main DLL within the source directory (used for checking existence)
dll_file_path: Path = source_plugin_dir / dll_file_name
...
def copy_dll_to_plugin_folder(instance_path: Path) -> None:
    ...
    if not discordbot_plugin_dir.is_dir():
        ...
        discordbot_plugin_dir.mkdir(parents=True, exist_ok=True) # Ensure parent dirs are created

    # Copy all DLLs from the source directory to the target plugin directory
    copied_files = 0
    logger.info(f'Copying all DLLs from {source_plugin_dir} to {discordbot_plugin_dir}')
    try:
        for item in source_plugin_dir.glob('*.dll'):
            if item.is_file():
                target_file = discordbot_plugin_dir / item.name
                logger.debug(f'Copying {item} to {target_file}')
                shutil.copy2(item, target_file)
                copied_files += 1
        if copied_files == 0:
            logger.warning(f"No DLL files found in source directory {source_plugin_dir} to copy.")
        else:
            logger.info(f"Copied {copied_files} DLL files successfully.")
    except Exception as e:
        logger.error(f"Error copying DLLs from {source_plugin_dir} to {discordbot_plugin_dir}: {e}")
...
if __name__ == "__main__":
    ...
    if not dll_file_path.is_file():
        logger.error(f"Source DLL not found at {dll_file_path}! Please ensure it exists.")
        exit(1)
    else:
        logger.info(f"Using source DLL from: {dll_file_path}")
